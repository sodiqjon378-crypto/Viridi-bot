...
def main_menu(is_admin=False):
    kb = [
        [KeyboardButton(text="ð Mahsulotlar"), KeyboardButton(text="â¹ï¸ Biz haqimizda")],
        [
            KeyboardButton(text="ð¤ Dillerlar uchun"),
            KeyboardButton(text="ð Bog'lanish"),
        ],
    ]
    if is_admin:
        kb.append([KeyboardButton(text="âï¸ Admin Panel")])
...
def admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="â Mahsulot qo'shish"), KeyboardButton(text="â Mahsulotni o'chirish")],
            [KeyboardButton(text="ð Mahsulotlar ro'yxati"), KeyboardButton(text="ð¥ Dillerlar arizalari")],
...
@router.message(F.text == "ð Asosiy menyu")
...
async def about_us(message: Message):
    await message.answer(
        "ð¿ **VIRIDI Group** â uy tozalash, kir yuvish, oshxona va vanna-tualet gigiena vositalarining O'zbekistondagi ishonchli yetkazib beruvchisi.\n\n"
        "â¨ **Bizning afzalliklarimiz:**\n"
        "â¢ Yuqori sifatli va samarali tozalash mahsulotlari;\n"
        "â¢ Uy bekalari va kafe-restoranlar uchun qulay tanlov;\n"
        "â¢ Hovlilar, kafel, bruschatka va marmarlarni chuqur tozalash hamda oqartirish vositalari;\n"
        "â¢ Chakana va ulgurji (dillerlik asosida) savdo tizimi;\n"
        "â¢ Hamyonbop narxlar.\n\n"
        "ð **Aloqa uchun telefon:** +998937413339\n"
        "ð¤ **Telegram menejer:** @um1daxon3339",
        parse_mode="Markdown",
    )


@router.message(F.text == "ð Bog'lanish")
async def contact_us(message: Message):
    await message.answer(
        "ð Murojaat uchun:\n\n"
        "ð¤ Telegram: @um1daxon3339\n"
        "ð± Telefon: +998937413339"
    )


@router.message(F.text == "ð¤ Dillerlar uchun")
async def dealer_section(message: Message):
    ...
    if res and res[0] == "approved":
        ...
        kb = ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text="ð± Telefon raqamni yuborish", request_contact=True)]],
            resize_keyboard=True,
        )
        await message.answer(
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"ð¤ Yangi diller arizasi!\nIsm: {message.from_user.full_name}\nTel: {contact.phone_number}",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[[InlineKeyboardButton(text="Tasdiqlash â", callback_data=f"approve_{message.from_user.id}")]]
                ),
            )
        except Exception:
            pass
...
async def approve_dealer(callback: CallbackQuery):
    ...
    conn.commit()
    await callback.message.edit_text("Diller tasdiqlandi â")
    await bot.send_message(user_id, "Tabriklaymiz! Sizning dillerlik arizangiz tasdiqlandi.")


@router.message(F.text == "ð Mahsulotlar")
async def show_categories(message: Message):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="ð³ Oshxona degresrlari", callback_data="cat_degreaser")],
            [InlineKeyboardButton(text="ð§º Kir yuvish vositalari", callback_data="cat_laundry")],
            [InlineKeyboardButton(text="ð½ Vanna va tualet", callback_data="cat_bathroom")],
            [InlineKeyboardButton(text="ð§¼ Suyuq sovunlar", callback_data="cat_soap")],
            [InlineKeyboardButton(text="ð¢ Kafe va restoranlar uchun", callback_data="cat_cafe")],
            [InlineKeyboardButton(text="ð§± Hovli, kafel va marmarlar", callback_data="cat_yard")],
        ]
    )
...
async def show_products_by_cat(callback: CallbackQuery):
    ...
    if not products:
        await callback.message.edit_text("Bu kategoriyada hozircha mahsulot yo'q.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="ð Orqaga", callback_data="back_cats")]]))
        return

    kb = []
    for p in products:
        kb.append([InlineKeyboardButton(text=f"{p[1]} â {p[2]} so'm", callback_data=f"prod_{p[0]}")])
    
    kb.append([InlineKeyboardButton(text="ð Orqaga", callback_data="back_cats")])
...
async def show_product_detail(callback: CallbackQuery):
    ...
    if p:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="ð Buyurtma berish", callback_data=f"order_{prod_id}")],
                [InlineKeyboardButton(text="ð Orqaga", callback_data="back_cats")],
            ]
        )
...
@router.message(F.text == "âï¸ Admin Panel")
...
async def process_description(message: Message, state: FSMContext):
    ...
    conn.commit()
    await state.clear()
    await message.answer("Mahsulot hajm va rasm/video bilan muvaffaqiyatli qo'shildi! â", reply_markup=admin_menu())


@router.message(F.text == "â Mahsulotni o'chirish")
async def start_delete_product(message: Message, state: FSMContext):
    if message.from_user.id in ADMIN_IDS:
        ...
        text = "â Qaysi mahsulotni o'chirmoqchisiz?\nIltimos, o'sha mahsulotning **ID raqamini** yozib yuboring (masalan: 1):\n\n"
        for p in products:
            text += f"ID: {p[0]} | {p[1]} â {p[2]} so'm\n"
...
async def process_delete_product(message: Message, state: FSMContext):
    if message.from_user.id in ADMIN_IDS:
        ...
        conn.commit()
        await state.clear()
        await message.answer(f"ID raqami {prod_id} bo'lgan mahsulot o'chirib yuborildi! ð", reply_markup=admin_menu())


@router.message(F.text == "ð Mahsulotlar ro'yxati")
